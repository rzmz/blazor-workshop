...
public static class PizzaApiExtensions
{
  public static WebApplication MapPizzaApi(this WebApplication app)
  {
      ...
      NotificationSubscription subscription) =>
    {
      ...
      IQueryable<NotificationSubscription> oldSubscriptions =
        db.NotificationSubscriptions.Where(e => e.UserId == userId);
    ...
    {
      List<PizzaSpecial> specials = await db.Specials.ToListAsync();
    ...
    {
      List<Topping> toppings = await db.Toppings.OrderBy(t => t.Name).ToListAsync();
  ...
  private static string GetUserId(HttpContext context) => context.User.FindFirstValue(ClaimTypes.NameIdentifier);
}